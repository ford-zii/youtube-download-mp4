...
def download_mp3():
    ...
    for link in links:
        try:
            ...
            subprocess.call(['ffmpeg', '-i', os.path.join(output_path, f"{video.title}.mp4"), os.path.join(output_path, f"{video.title}.mp3")])
            
            # Remove the original downloaded file
            os.remove(os.path.join(output_path, f"{video.title}.mp4"))
            
            print(f"{video.title}.mp3 downloaded successfully!")
